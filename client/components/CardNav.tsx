    ...
    >
      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 h-[60px] flex items-center justify-between px-3 z-[2]">
          ...
          <button
            type="button"
            className="card-nav-cta-button hidden md:inline-flex border-0 rounded-full px-6 py-2 items-center font-medium cursor-pointer transition-all duration-300 hover:shadow-lg hover:shadow-current/30 active:scale-95"
        ...
        >
          {(items || []).slice(0, 3).map((item, idx) => (
            <div
              key={`${item.label}-${idx}`}
              className="nav-card select-none relative flex flex-col gap-3 p-4 rounded-xl min-w-0 flex-[1_1_auto] h-auto min-h-[70px] md:h-full md:min-h-0 md:flex-[1_1_0%] shadow-md hover:shadow-lg transition-shadow duration-300"
              ref={setCardRef(idx)}
            ...
            >
              ...
              <div className="nav-card-links mt-auto flex flex-col gap-2">
                {item.links?.map((lnk, i) => (
                  <a
                    key={`${lnk.label}-${i}`}
                    className="nav-card-link inline-flex items-center gap-2 no-underline cursor-pointer transition-all duration-300 hover:opacity-80 hover:translate-x-1 text-[14px] md:text-[15px]"
                    href={lnk.href}
                    aria-label={lnk.ariaLabel}
                  >
                    <GoArrowUpRight
                      className="nav-card-link-icon shrink-0 text-[16px]"
                      aria-hidden="true"
                    />
                    {lnk.label}
                  </a>
